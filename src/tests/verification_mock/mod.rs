...
mock_root_test_cases! {
    ...
    wrong_eku_dns [ any(target_os = "linux") ] => TestCase {
        reference_id: EXAMPLE_COM,
        chain: &[include_bytes!("root1-int1-ee_example.com-wrong_eku.crt"), ROOT1_INT1],
        stapled_ocsp: None,
        expected_result: Err(TlsError::InvalidCertificate(
            CertificateError::Other(Arc::from(Box::from(webpki::Error::RequiredEkuNotFound))))),
    },
    wrong_eku_dns [ any(windows, target_os = "android", target_os = "macos") ] => TestCase {
        reference_id: EXAMPLE_COM,
        chain: &[include_bytes!("root1-int1-ee_example.com-wrong_eku.crt"), ROOT1_INT1],
        stapled_ocsp: None,
        expected_result: Err(TlsError::InvalidCertificate(
            CertificateError::Other(Arc::from(Box::from(error_messages::INVALID_EXTENSIONS))))),
    },
